...
module fsi
  ...
  PUBLIC :: init_fsi, boundary_conditions_fsi, postprocess_fsi, &
            geomcomplex_fsi, visu_fsi, visu_fsi_init

contains

  subroutine geomcomplex_fsi(epsi,nxi,nxf,ny,nyi,nyf,nzi,nzf,dx,yp,remp)
    ...
    character(len=:), allocatable :: filegeoname
    zeromach=one
    ...
    ! Update center of moving object
    !cexx=cex+ubcx*t
    !ceyy=cey+ubcy*t
    ! Update center of moving object
    if (t.ne.0.) then
...
!
    ...
    return
  end subroutine geomcomplex_fsi

  !********************************************************************
  subroutine boundary_conditions_fsi (ux,uy,uz,phi)
    ...
    return
  end subroutine boundary_conditions_fsi
  !********************************************************************
  ...
  !********************************************************************
  subroutine init_fsi (ux1,uy1,uz1,phi1)
  ...
  !############################################################################
  subroutine postprocess_fsi(ux1,uy1,uz1,ep1)
  ...
  end subroutine postprocess_fsi

  subroutine visu_fsi_init (visu_initialised)
    ...
    call decomp_2d_register_variable(io_name, "epsi", 1, 0, output2D, mytype)

    visu_initialised = .true.
    
  end subroutine visu_fsi_init
  !############################################################################
  !!
  !!  SUBROUTINE: visu_fsi
  !!      AUTHOR: FS
  !! DESCRIPTION: Performs object-specific visualization
  !!
  !############################################################################
  subroutine visu_fsi(ux1, uy1, uz1, pp3, phi1, ep1, num)
    ...
    call write_field(ep1, ".", "epsi", num, flush = .true.)

  end subroutine visu_fsi
